...
public class Concord {
   ...
   public void printSorted(PrintStream out) {
      ArrayList<Map.Entry<String, Integer>> arr = new ArrayList<>(concord.entrySet());
      // arr.sort(Collections.reverseOrder(Map.Entry.comparingByValue()));
      //arr.sort(Collections.reverseOrder(new ComparyByMapValueComparator()));
      Collections.sort(arr, Collections.reverseOrder(new ComparyByMapValueComparator()));
      for (Map.Entry<String, Integer> curr : arr) {
         out.println(curr.getKey() + " " + curr.getValue());
      }
   }
...
class ComparyByMapValueComparator implements Comparator<Map.Entry<String, Integer>> {

   public int compare(Map.Entry<String, Integer> entry1, Map.Entry<String, Integer> entry2) {
      int valueDiff = entry1.getValue() - entry2.getValue();
      if (valueDiff != 0) {
         return valueDiff;
      } else {
